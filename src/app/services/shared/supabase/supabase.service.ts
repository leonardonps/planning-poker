...
export class SupabaseService {  
  ...
  async inserirSessao(sessao: ISessao): Promise<void> {
    const { error } = await this.supabase.from('sessao').upsert([{id: sessao.id, opcoes_estimativa: sessao.opcoesEstimativa}]).select().single();
  ...
  async inserirUsuario(usuario: IUsuario): Promise<void> {   
    const { error } = await this.supabase.from('usuario').upsert([{id: usuario.id, nome: usuario.nome, observador: usuario.observador, sessao_id: usuario.sessaoId, presence_id: usuario.presenceId}]).select().single();
  ...
  async atualizarPresence(usuarioId: string, presence: string): Promise<void> {
    const { error } = await this.supabase.from('usuario').update({presence_id: presence}).eq('id', usuarioId);
